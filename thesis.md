...
**종합**: 이 파일럿은 10~11차의 세 교란 변수 중 하나(비매칭 표본)만 제거한 것이라 "5W1H 원칙이 최종적으로 효과가 없다"는 결론을 내리기엔 여전히 이르다 — 오히려 매칭 표본으로도 round_1/who/why 가설이 계속 지지되지 않는다는 점은, 원래 우려했던 "비매칭 표본이 원인이었을 수 있다"는 가능성을 약화시키고, 5W1H 조항 자체의 효과가 실제로 제한적일 가능성 쪽으로 조금 더 무게를 싣는다. Meta-Rule 개수 감소만은 유일하게 긍정적 방향으로 나왔으나 라운드 수가 너무 적어 확정하기 어렵다. 본 실행(카테고리당 5개, n_repeat=5, 더 많은 라운드)에서 최종 확인이 필요하다.
