...
2. ~~**[선행 권장] 모델 drift 사전 탐지용 "카나리아" 절차**~~ → **[2026-08-13] 구현 완료(항목 33).** `canary.py::check_canary_drift()` — 추가 API 호출 없이 `usage_summary`를 저장된 기준선과 비교만 함(`--canary-baseline`/`--save-canary-baseline`). **아직 하지 않은 것**: 실제 정상 실행으로 최초 기준선(`data/canary_baseline.json`)을 만드는 것 — 본 실행 시 `--save-canary-baseline`로 처음 한 번 생성해야 이후 실행부터 비교가 의미를 가진다.
...
6. ~~4단계 Level 체계(§4.1.1)를 실제 데이터에 적용해 심판관 신뢰도 재확인~~ → **[2026-08-13] 코드 구현 완료(항목 38).** `judge.py` rubric을 4단계(FULL_DEFENSE/FUNCTIONAL_DEFENSE/PARTIAL_EXPOSURE/BREACH)로 갱신, `experiment.py`/`stats.py`/`run_experiment.py`/`build_trace_report.py`/`cross_judge_check.py`/`spotcheck_sample.py`까지 파이프라인 전체에 전파, pytest 16건·mock 스모크(콘솔+HTML)로 검증 완료. **아직 하지 않은 것**: 실제 API 데이터로 심판관 신뢰도(교차 심판관 재검증)를 이 새 4단계 기준으로 다시 확인하는 것 — 본 실행에서 함께 수행 예정.
