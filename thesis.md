...
1. **[선행 권장] N=5 반복 시행을 실제로 구현할지, 아니면 N=1로 진행해온 것을 한계로 명시할지 결정** — §3.5.1이 명시했으나 1~11차 전체 미구현. 라운드 진동이 실제 신호인지 노이즈인지 가르는 가장 근본적인 공백(항목 28). 구현 시 비용 최대 5배 증가.
2. **[선행 권장] 모델 drift 사전 탐지용 "카나리아" 절차 도입** — 매 실행 시작 시 고정 문항 몇 개를 먼저 돌려 과거 기준치(토큰량·거부율)와 자동 대조. 구현 난이도 낮음(항목 28, KB 스냅샷과 동일 패턴 재사용 가능).
3. **[선행 권장] 공격 시나리오 풀을 캐싱해 baseline/변형 비교를 진짜 대응표본으로 전환** — `AttackScenario`를 JSON으로 저장·재사용하는 `--attack-pool-file` 옵션 추가(항목 28, 구현 난이도 낮음~중간). 단, 적응형 재공격은 설계상 매번 새로 생성되므로 이 캐싱으로도 완전히 해결되지 않는 부분이 남음.
4. **[선행 권장] Meta-Rule 샌드위치 경량화 — 최소한 심판관 프롬프트에서 이중 삽입 제거** — 같은 실행 내부에서도 라운드15 시점 입력 토큰이 4.6~4.9배로 실측됨(치유 루프 입력 비용의 63.9%, 항목 28). 비용 절감 효과가 가장 큼.
5. **baseline vs 5w1h 재실행 여부 결정** — 위 1~4를 해결한 뒤 진행하는 것을 권장(항목 27, 28)
6. 4단계 Level 체계(§4.1.1)를 실제 데이터에 적용해 심판관 신뢰도 재확인 (항목 24)
7. 헬드아웃 셋을 치유용 셋과 완전히 독립된 배치로 생성할지 결정 — 현재는 같은 1차 생성 배치를 무작위로 나눔 (항목 4, §6)
8. 벤더 격차-SRS 성숙도 상관관계 가설을, 동일 SRS 고정 상태로 성숙도만 다르게 해 직접 검증 (항목 22, §5.3.9) — 단, §5.3.9가 비교한 7~9차도 매 실행 공격 문항이 새로 샘플링된 비매칭 표본이었다는 점이 항목 28에서 추가 확인됨, 이 가설 검증도 위 B.3(공격 풀 캐싱)이 선행되어야 깨끗해짐
9. 카테고리당 10개 규모의 "본 실행" — 이번 3개 규모 실행이 예상보다 비쌌던 원인이 drift만이 아니라 Meta-Rule 샌드위치 구조 자체(모든 실행에 공통, B.4)로도 확인됐으므로, B.4 적용 후 비용을 재추정하고 예산 확정 필요 (항목 27, 28, 부록 H)
...
2. 향후 모든 실행에 "실행 간 평균 output 토큰(역할별)" 비교를 재현성 점검 절차로 추가 — 모델 동작 변화(drift) 조기 감지용 (항목 27, §3.5.2) — 위 B.2 카나리아 절차로 구체화
...
5. **`generate_meta_rules` 호출에 예외 처리 추가** — judge 호출과 달리 try/except가 없어, 라운드 후반(가장 비싼 시점)에 연속 실패하면 그때까지의 전체 결과가 크래시와 함께 유실될 위험(항목 28). 코드 몇 줄 수준.
6. **라운드 단위 부분 결과 체크포인트 저장** — 현재 전체 결과가 실행 종료 시 한 번에만 저장돼 C.5와 같은 크래시 시 전손 위험이 있음(항목 28)
7. **`config.py`의 죽은 temperature 설정 정리** — `.env`의 `LLM_TEMPERATURE`/`JUDGE_TEMPERATURE`를 실제로 참조하거나, 코드에 하드코딩된 값이 진짜 값임을 문서에 명시(항목 28)
8. **API 오류/재시도 호출의 토큰을 비용 집계에 정확히 반영** — 현재 예외 발생 시 토큰이 `None`으로 기록돼 비용 추정치에서 누락됨(항목 28) — §5.3.10/부록 H의 비용 수치 정밀도에 직결
9. 유닛C의 결정론적 거절 폴백 문구(`_REFUSAL_MESSAGE`)가 SRS 자체 규칙(추가 질문 금지)을 위반하도록 하드코딩돼 있어 인위적 FAIL을 만들어내는 문제 — 수정하되 과거 1~11차 결과와의 비교가능성에 영향을 준다는 점을 명시하고 신중히 진행 (항목 28)

**D. 문헌·인용 보강 (항목 28, §2.3/§2.4에 1차 반영 완료 — 원문 확인 및 세부 보강 필요)**
1. PRISM(2026)·SISF(2025) 원문을 직접 확인해 §2.4 비교표의 "미상" 항목을 채우고 인용 정확성 확정
2. 레드팀/적응형 재공격 문헌에 PAIR·TAP(블랙박스)·GCG·AutoDAN(화이트박스) 추가 인용 검토 (§5.4)
3. Promptware Engineering(Chen et al., 2025)을 §1.1/§2.1에 인용해 "SRS 프레이밍"의 계보를 명시
4. LLM-as-judge 신뢰성 논의(§3.6)에 Zheng et al.(MT-Bench/Chatbot Arena, NeurIPS 2023), Panickssery et al.(self-preference bias, NeurIPS 2024) 추가
5. LLM API 행동 drift 논의(§3.5.2/§5.3.10)에 Chen, Zaharia, Zou(2023, "How is ChatGPT's behavior changing over time?") 인용 추가 — 본 연구가 실측한 현상의 선행 근거
6. §2.1의 "[TODO: 정확한 인용 확인]"으로 비어있던 NeMo Guardrails 인용을 Rebedea et al.(EMNLP 2023 Demos, arXiv:2310.10501)로 채움
7. OWASP LLM Top 10이 2026-08-04에 2026판(v2.0에서 갱신)으로 개정됨 — §4.1.1/§4.2.1의 인용 버전을 명시하거나 2026판 대조표 검토
