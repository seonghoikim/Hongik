...
2. Level 체계의 정확한 단계 수(3단계 유지 vs 4단계, 현재는 연구자가 4단계로 잠정 설계함) 및 참고할 선행 연구 존재 여부 확정 (항목 23, 24)
3. 5W1H를 "심판관/SRS의 구조화된 판단 프레임워크"로 반영한 방향이 지도교수 의도와 맞는지 확인 — 특히 항목 27에서 가설이 기각된 실측 결과까지 포함해 다시 여쭤볼 필요 (항목 23, 25, 27)
4. SRS 페르소나 성별("친절한 여성 직원") 유지 여부 (항목 6)
5. 회사 실명(롯데하이마트 PLUX) 노출 여부 — 그대로 vs 익명화 (항목 16)

**B. 실험 재검증·확장**
1. **baseline vs 5w1h 재실행 여부 결정** — 가설이 기각됐지만 교란 변수(작은 표본·비매칭 공격셋·모델 drift)가 섞여 있어 확정적 결론이 아님(§5.3.10). 재실행한다면 (i) 모델 동작이 상대적으로 안정된 시점에, (ii) 동일 공격 문항을 baseline/5w1h 양쪽에 재사용하는 진짜 대응표본 설계로, (iii) 표본을 늘려서 진행 (항목 27)
2. 4단계 Level 체계(§4.1.1)를 실제 데이터에 적용해 심판관 신뢰도 재확인 (항목 24)
3. 헬드아웃 셋을 치유용 셋과 완전히 독립된 배치로 생성할지 결정 — 현재는 같은 1차 생성 배치를 무작위로 나눔 (항목 4, §6)
4. 벤더 격차-SRS 성숙도 상관관계 가설을, 동일 SRS 고정 상태로 성숙도만 다르게 해 직접 검증 (항목 22, §5.3.9)
5. 카테고리당 10개 규모의 "본 실행" — 이번 3개 규모 실행이 예상보다 2배 비쌌으므로(항목 27), 본 실행 비용을 이 실측치 기준으로 재추정 후 예산 확정 필요

**C. 파이프라인/방법론 보완**
1. 심판관 거부율 급증(9차 6.8%→10차 12.6%) 원인을 raw_calls 로그로 더 깊이 조사 — 특정 카테고리(encoding_bypass) 편중 여부 재확인 (항목 27, §3.6.3)
2. 향후 모든 실행에 "실행 간 평균 output 토큰(역할별)" 비교를 재현성 점검 절차로 추가 — 모델 동작 변화(drift) 조기 감지용 (항목 27, §3.5.2)
...
3. 제출 전 이 절과 "⏳ 지도교수 확인 대기 목록" 절 전체를 삭제하고, 확정된 내용만 본문에 남기기
