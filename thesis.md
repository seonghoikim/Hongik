...
**A. 지도교수 확인 필요 (다음 미팅)**
1. 제목을 "프롬프트를 정의하는 메커니즘" 쪽으로 실제로 바꿀지 여부 (항목 23)
2. Level 체계의 정확한 단계 수(3단계 유지 vs 4단계, 현재는 연구자가 4단계로 잠정 설계함) 및 참고할 선행 연구 존재 여부 확정 (항목 23, 24) — Regmi & Saravanan(2025/2026)이 유사한 4단계 챗봇 응답 분류를 이미 제안했음을 발견(항목 28), 이를 어떻게 자리매김할지도 함께 논의
3. 5W1H를 "심판관/SRS의 구조화된 판단 프레임워크"로 반영한 방향이 지도교수 의도와 맞는지 확인 — 특히 항목 27에서 가설이 기각된 실측 결과까지 포함해 다시 여쭤볼 필요 (항목 23, 25, 27)
4. **"자가 치유 폐쇄 루프"를 논문의 "가장 핵심적인 기여"로 서술해온 것을 재조정한 방향(§1.3, §2.3, 항목 28)이 맞는지 확인** — PRISM·SISF 등 매우 유사한 선행연구가 확인되어, 신규성 주장을 "골격의 최초 고안"에서 "구체적 조합 + 실제 운영 도메인에서의 정직한 실패 실증"으로 좁혔음. 지도교수 관점에서 이 재조정 방향이 충분한지, 혹은 §1.3 기여도 목록 자체를 더 크게 손봐야 하는지 상의 필요
5. SRS 페르소나 성별("친절한 여성 직원") 유지 여부 (항목 6)
6. 회사 실명(롯데하이마트 PLUX) 노출 여부 — 그대로 vs 익명화 (항목 16) — §4.1.1에 실명이 이미 한 차례 노출된 상태(항목 28 점검에서 발견), 결정되는 대로 일괄 정리 필요
...
2. ~~**[선행 권장] 모델 drift 사전 탐지용 "카나리아" 절차**~~ → **자동화된 게이트는 아직 미구현**(수동으로 실행 간 `usage_summary.by_provider_role` 비교는 가능). 후속 과제로 남김.
...
5. **baseline vs 5w1h 재실행 여부 결정** — 위 1·3·4가 해결됐으므로 재실행 인프라는 준비됨. 실제 재실행은 API 버전 고정(§F 신설 예정) 결정 이후 진행 권장.
6. 4단계 Level 체계(§4.1.1)를 실제 데이터에 적용해 심판관 신뢰도 재확인 (항목 24)
7. 헬드아웃 셋을 치유용 셋과 완전히 독립된 배치로 생성할지 결정 — 현재는 같은 1차 생성 배치를 무작위로 나눔 (항목 4, §6)
8. 벤더 격차-SRS 성숙도 상관관계 가설 검증 — 이제 B.3(공격 풀 캐싱)으로 동일 공격 문항 재사용이 가능해졌으므로 다음 실행부터 시도 가능 (항목 22, §5.3.9)
9. 카테고리당 10개 규모의 "본 실행" — B.4(Meta-Rule 경량화)로 비용 일부 절감 예상되나, N=반복시행을 적용하면 오히려 그만큼 비용이 늘어나므로(B.1) 두 효과를 함께 반영해 재추정 필요 (항목 27, 28, 부록 H)

**C. 파이프라인/방법론 보완**
1. 심판관 거부율 급증(9차 6.8%→10차 12.6%) 원인을 raw_calls 로그로 더 깊이 조사 — 특정 카테고리(encoding_bypass) 편중 여부 재확인. 단, unit_c 거부(정책상 자체 거부)와 judge 거부가 결정 로그 항목 27에서 하나의 지표로 섞여 서술된 점도 함께 분리해서 봐야 함(항목 28)
2. 향후 모든 실행에 "실행 간 평균 output 토큰(역할별)" 비교를 재현성 점검 절차로 추가 — 자동 게이트는 B.2 참조(아직 미구현), 수동 비교는 가능
3. §4.2.2에서 설계한 "연구자 수동 검토(`manual_review_hook`)" 단계가 지금까지 모든 실행에서 기본값(전부 통과)으로 실제 수행되지 않음 — 논문 제출 전 실제 수행 필요
4. §3.6.2 심판관 신뢰성 검증의 연구자 본인 수동 블라인드 채점(`spotcheck_sample.py`/`spotcheck_compare.py`)이 아직 미수행 — 교차 심판관 재검증(항목 10)으로 정황 증거만 확보한 상태
...
1. PRISM(2026)·SISF(2025) — 아직 **원문 미확인**(검색 스니펫·2차 소스로만 교차 확인). §2.4 비교표의 "미상" 항목은 원문 확인 후 채울 것.
...
7. OWASP LLM Top 10 2026판 반영 — **아직 미반영**, §4.1.1/§4.2.1은 여전히 2025판(v2.0) 기준. 원문 대조 후 갱신 필요.
...
**E. 문서·논문 정리**
1. thesis.md 제목·초록·본문에 아직 남아있는 "가전 유통 A사" 서술을 실제 도메인(롯데하이마트 PLUX)으로 전부 갱신 (항목 16)
2. `지도교수_피드백_히스토리_로그.docx`에 다음 미팅 결과 반영해 이어서 기록
3. 제3장에 3.1~3.4 절 번호가 비어 있음(3.5로 바로 시작) — 번호 복구 또는 재번호 (항목 28)
4. 표 캡션 형식 통일 — "표 X. 교차 모델 검증 결과"만 유일하게 캡션이 있고 번호 플레이스홀더도 안 채워짐, 나머지 표는 캡션 없음 (항목 28)
...
> ⚠️ **재현성 공백의 실측 사례 — 모델 동작 변화(drift) (2026-08-12 확인, §5.3.10 상세)**: 위 표에서 이미 "Anthropic API가 별도 날짜 스냅샷 문자열을 반환하지 않는다"는 한계를 지적해 두었는데, 이것이 이론적 우려에 그치지 않고 실제로 결과에 영향을 준 사례를 확보했다. 9차 실행(2026-08-04)과 8일 뒤 동일 코드·동일 설정으로 돌린 10차 실행(2026-08-12)의 원문 API 로그를 대조한 결과, 코드를 건드리지 않은 역할(unit_c)까지 포함해 anthropic 세 역할(judge·unit_c·meta_rule_gen) 전부에서 응답 텍스트 길이는 별로 늘지 않았는데 output 토큰 소모량이 4~7배 뛰었다 — extended thinking에 소모되는 토큰이 서버 측에서 늘어난 것으로 추정된다(§5.3.10). `model_version` 필드는 이번에도 두 실행 모두 `claude-sonnet-5`로 동일해, **API가 반환하는 버전 문자열만으로는 이 변화를 감지할 수 없었다.** 이후 실행부터는 `usage_summary.by_provider_role`의 역할별 평균 output 토큰(호출 수 대비 output_tokens)을 실행 간 비교해 "같은 별칭이 실제로도 같게 행동하고 있는가"를 확인하는 것을 재현성 점검 절차에 추가할 것을 권고한다. git commit 해시(§3.5.6)는 "우리 쪽 코드가 그대로였다"는 것만 보장할 뿐, "모델이 그대로였다"는 것은 보장하지 못한다는 점이 이번에 실증됐다. 이 현상 자체는 본 연구가 처음 관측한 것이 아니라 이미 선행 연구로 정량화된 바 있다 — Chen, Zaharia, Zou("How is ChatGPT's behavior changing over time?," 2023, arXiv:2307.09009, Harvard Data Science Review)는 GPT-3.5/GPT-4의 3월판과 6월판이 소수 판별·코드 생성 등 여러 과제에서 크게 다른 성능을 보임을 실측해, "모델이 언제 어떻게 갱신되는지가 불투명하다"는 문제를 최초로 정량화했다(2026-08-12 문헌 재조사, 결정 로그 항목 28). 본 연구의 §5.3.10 발견은 이 현상이 8일이라는 짧은 간격에서도, 그리고 output 토큰 소모량이라는 새로운 지표에서도 재현됨을 보여주는 추가 실증 사례다.
...
> 이 절은 지도교수 피드백(결정 로그 항목 23)을 반영해 위 3단계 Action Matrix를 일반화·확장한 **제안**이다. 지금까지 보고된 1~9차 파일럿 결과(§5.3)는 모두 위 3단계(PASS/WARNING/FAIL) 기준으로 채점되었으며, 아래 4단계 체계는 본 실행(§4.2.1) 또는 별도 검증 라운드에서 실제로 적용·검증할 예정인 확장안이다.

**문제의식.** 위 Action Matrix의 WARNING 판정 기준("간결한 단답형" 미준수, 지정 어투 이탈 등)은 실제로는 §5.1의 특정 고객사(롯데하이마트 PLUX) SRS 문구를 예시로 삼아 작성된 것이었다 — 이 케이스 스터디 밖에서는 그대로 재사용하기 어려운 도메인 특화 기준이다. 지도교수는 "레벨은 보안 등급과 같은 개념이며 레벨이 높을수록 완결성이 좋다"고 언급하며, 참고할 선행 연구가 있으면 인용하고 없으면 직접 정의·제안할 것을 요구하였다.
...
- **원인 진단**: 9차(2026-08-04)와 이번 실행(2026-08-12)의 원문 API 로그(`raw_calls_*.jsonl`)를 역할별로 대조한 결과, **응답 텍스트 길이는 별로 늘지 않았는데 output 토큰 소모량만 4~7배 뛰었다.**
  ...
  세 역할 모두 서로 다른 코드 경로이고, 이번 실행에서 코드 변경이 있었던 곳은 judge.py의 루브릭(§4.1.2, `exploited_axis` 필드 하나 추가)뿐이었는데도 unit_c·meta_rule_gen까지 동일하게 배율이 뛴 점은 코드 변경만으로는 설명되지 않는다. 두 실행의 `environment` 스냅샷(git commit, anthropic SDK 버전 `0.120.2` 등)을 대조해도 SDK·패키지 버전은 동일했다. 가장 유력한 설명은 **Anthropic API가 반환하는 모델 식별자가 `claude-sonnet-5`라는 별칭(alias)일 뿐 날짜가 박힌 스냅샷 문자열이 아니라는, §3.5.2에서 이미 지적했던 재현성 공백이 실제로 발현된 것** — 즉 같은 별칭이 가리키는 실제 가중치/기본 동작(특히 extended thinking에 소모하는 토큰량)이 8일 사이에 서버 측에서 바뀌었을 가능성이다. 확답할 수는 없지만(모델 버전 문자열 자체가 이를 구분해 주지 않으므로), 심판관 거부율 급증(baseline 124/983=12.6%, §3.6.3에 "드물게 관찰"이라 적어둔 것보다 훨씬 잦음)과 라운드가 갈수록 나빠지는 이번 실행의 이례적 패턴 모두 같은 원인(모델이 다르게 행동하기 시작함)으로 설명 가능하다는 정황 증거다.
...
4. **자체 점검한 추가 내적 타당성 위협 (Self-audited threats to validity)**: 심사 과정에서 지적될 수 있는 지점을 저자가 먼저 명시한다.
   ...
   - **모델 동작 변화(drift)가 재현성을 위협한다는 실측 사례 확보 — 8일 간격으로 재현되지 않은 9차 vs 10차 결과 (2026-08-12)**: §3.5.2에서 이론적 우려로만 적어 두었던 "API가 모델의 날짜 스냅샷을 반환하지 않아 재현성이 완전히 보장되지 않는다"는 한계가, 실제로 결과에 영향을 준 사례로 확인됐다(§5.3.10). 동일 코드·동일 파이프라인·동일 규모(카테고리당 3개)를 8일 간격(2026-08-04→2026-08-12)으로 재실행했더니, (1) anthropic 세 역할(심판관·유닛 C·Meta-Rule 생성기) 모두에서 응답 텍스트 길이는 비슷한데 output 토큰 소모량만 4~7배 증가했고, (2) 심판관 정책상 거부율이 급증했으며(9차 62/907=6.8% → 10차 124/983=12.6%, 이 중 상당수가 §3.6.3에서 "드물게 관찰"이라 적어둔 거부 현상), (3) 준수율 추이가 7~9차의 "진동하지만 결국 수렴"하는 패턴 대신 "라운드가 갈수록 통계적으로 유의하게 악화"(Wilcoxon p=0.0016, 방향은 저하)되는 패턴으로 바뀌었다. `model_version` API 응답 필드는 두 실행 모두 `claude-sonnet-5`로 동일해 이 변화를 코드만으로는 감지할 수 없었다. 이는 본 연구가 §3.5.2에서 "git commit 해시로 코드 버전을 고정한다"고 주장한 재현성 확보 조치가 **모델 자체의 시간에 따른 동작 변화까지는 통제하지 못한다**는 것을 실증하며, 상용 LLM API를 실험 인프라로 쓰는 모든 연구가 구조적으로 안고 있는 위협이다. §3.5.2에 실행 간 평균 output 토큰 비교를 재현성 점검 절차로 추가하도록 권고를 남겼고, §4.1.2의 5W1H 가설 검증(§5.3.10)이 이 교란 변수와 겹쳐 결과 해석이 더 복잡해졌다는 점도 함께 기록한다.
