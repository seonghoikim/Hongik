...
13. **[2026-08-04] RAG 지식베이스 버전 스냅샷 도입**: 연구자 피드백 — "무슨 내부 정보를 RAG로 당겨와서 저렇게 답했는지 알 도리가 없다." SRS는 라운드마다 `data/srs/*.json`으로 자동 저장되지만 KB(`knowledge_base.py`)는 그동안 코드에만 있어서 실행 시점의 전체 내용을 결과 JSON만으로 재구성할 수 없었음. `KB_VERSION`(현재 `kb_v1`)을 명시하고 `run_experiment.py` 실행 시작 시 전체 KB를 `data/rag/{KB_VERSION}.json`으로 스냅샷 저장하도록 함(SRS 저장 방식과 동일 목적). `experiment_*.json`에 `kb_version` 필드를 추가하고, `build_trace_report.py`에 실행 시 사용된 전체 KB 항목(트리거 키워드 전체 목록 포함)을 보여주는 절을 신설함. KB 항목을 의미 있게 바꿀 때마다 `KB_VERSION`을 올려 이력을 추적한다.
