...
5. **baseline vs 5w1h 재실행 여부 결정** — 위 1·3·4가 해결됐으므로 재실행 인프라는 준비됨. **[2026-08-13]** 연구자: "이건 할일로 일단 킵. 테스트 부분이니까 좀더 검토하고 실행하자" — 아래 B.9(본 실행 규모) 결정 이후 함께 진행.
6. 4단계 Level 체계(§4.1.1)를 실제 데이터에 적용해 심판관 신뢰도 재확인 (항목 24) — **[2026-08-13]** 단계 수 자체는 4단계로 확정(항목 32), 실제 데이터 적용은 본 실행과 함께 진행.
7. 헬드아웃 셋을 치유용 셋과 완전히 독립된 배치로 생성할지 결정 — 현재는 같은 1차 생성 배치를 무작위로 나눔 (항목 4, §6). **[2026-08-13]** 연구자가 독립/비독립 각각의 장단점 설명을 요청함(항목 32) — 본문(§6 한계점 인접)에 장단점 비교 추가 후 연구자가 결정.
8. 벤더 격차-SRS 성숙도 상관관계 가설 검증 — 이제 B.3(공격 풀 캐싱)으로 동일 공격 문항 재사용이 가능해졌으므로 다음 실행부터 시도 가능 (항목 22, §5.3.9)
9. 카테고리당 N개 규모의 "본 실행" — B.4(Meta-Rule 경량화)로 비용 일부 절감 예상되나, N=반복시행을 적용하면 오히려 그만큼 비용이 늘어나므로(B.1) 두 효과를 함께 반영해 재추정 필요 (항목 27, 28, 부록 H). **[2026-08-13]** 연구자 문제 제기: 연구비 지원이 없어 비용에 민감함 — "n=3과 n=5의 차이가 크지 않을 것 같다면 n=3으로, 정확도 우려가 있다면 n=5 유지. 카테고리당 10개가 큰 의미가 있나? n_repeat=5 반복 검증 + 헬드아웃 교차검증까지 하는데?" → **분석·권고 완료(부록 H, 항목 32)**: n_repeat은 라운드당 비용을 5배가 아니라 실측상 약 1.8배만 늘리고 통계적 이득이 커서 **5 유지 권고**, 카테고리당 개수는 n_repeat·헬드아웃과 통제하는 위협이 다르므로(노이즈 vs 커버리지) 10 대신 **5로 절충** 권고 — 종합 추정 비용 약 $17~34(추정치, 실측 아님). 연구자 최종 결정 대기.
