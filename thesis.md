...
4. **자체 점검한 추가 내적 타당성 위협 (Self-audited threats to validity)**: 심사 과정에서 지적될 수 있는 지점을 저자가 먼저 명시한다.
   ...
   - **Meta-Rule 반복 보강만으로는 완전한 수렴에 도달하지 못하며, 8차 실행에서는 비단조적 진동까지 관측됨**: §5.3에서 관측된 것처럼, 동일한 지시("추가 질문을 하지 말라")를 5회 표현만 바꿔 반복 삽입했음에도 7차 round_5가 88.4%에서 멈췄다(round_4는 오히려 79.7%로 하락). §5.3.7(8차 실행)에서는 더 뚜렷하게, round_1(71.2%)→round_2(75.8%) 개선 후 round_3에서 63.6%(최저, FAIL 6건)로 급락했다가 round_5(71.2%)로 겨우 회복했고, round_1 대 round_5의 Wilcoxon 검정은 p=1.000으로 순유의미한 개선이 없었다. 이는 자가 치유 메커니즘이 "무엇이 문제인지 정확히 진단하는" 능력과 "그 진단을 완전히·안정적으로 해소하는" 능력이 별개일 수 있음을 시사한다 — 특히 누적된 Meta-Rule 사이에 서로 다른 상황에 적용되는 조건(예: "부연설명 금지" vs "제품 결함 관련 거절은 공감 표현 필수")이 늘어날수록, 대상 행동이 RLHF로 깊게 학습된 대화 스타일 기본값과 충돌하며 일시적으로 더 나빠지는 구간이 나타날 수 있다. `max_rounds`를 늘려 더 많은 라운드를 시도했을 때 이 진동이 실제로 수렴하는지, 아니면 결정론적 출력 후처리(유닛 D 강화) 같은 비-프롬프트 메커니즘이 추가로 필요한지는 아직 확인되지 않았으며 후속 과제로 남긴다(연구자 확인 대기 목록 항목 20).
   - **표본 크기의 한계**: §5.3/§5.3.7의 결과는 두 실행 모두 카테고리당 3개(총 23~24개 문항)의 예비 규모이며, 논문이 원래 계획한 카테고리당 10개 규모의 본 실행은 아직 수행하지 않았다. 적응형 재공격에서 블랙박스/화이트박스 방어율의 대소 관계가 7차(93.3%<100%)와 8차(60.0%<80.0%)에서는 원래 가설과 반대·일치로 서로 다르게 나온 것(§5.3-(4), §5.4)은 표본이 각 5개에 불과한 데서 오는 통계적 불안정성일 가능성이 크며, 본 실행에서 재확인이 필요하다.
