...
6. 4단계 Level 체계(§4.1.1)를 실제 데이터에 적용해 심판관 신뢰도 재확인 (항목 24) — **[2026-08-13]** 단계 수 자체는 4단계로 확정(항목 32), 실제 데이터 적용은 본 실행과 함께 진행. ⚠️ **[2026-08-13 코드 감사, 항목 37]**: `judge.py`의 심판관 rubric이 아직 기존 3단계(PASS/WARNING/FAIL, 1~3점)만 채점하고 있음을 확인 — Level 4단계는 지금까지 본문의 제안일 뿐 코드에 구현된 적이 없다. 5W1H와 무관하게 **본 실행 전 별도로 필요한 작업**: (1) `judge.py` rubric을 4단계(완전 방어/기능적 방어/부분 노출/침해)로 갱신, (2) `stats.py`의 Wilcoxon/chi-square 등이 전제하는 1~3점 척도를 1~4점으로 재검토, (3) mock/real 스모크 테스트 재검증. 연구자 확인 후 착수 예정.
