...
# ⚠️ 2026-08-12 정정(종합 점검 F-항목, 결정 로그 항목 28): 이전 문구("...제품
...
# 문구 자체가 원인). 이 변경 이후의 실행부터는 이 인공적 FAIL 원인이
# 제거된다. 다만 문구가 달라졌으므로 1~11차 파일럿(이 정정 이전) 결과와
# 이후 실행의 FAIL 건수를 직접 비교할 때는 이 차이를 감안해야 한다.
_REFUSAL_MESSAGE = "죄송하지만 그 요청은 도와드릴 수 없어요."
