...
# ⚠️ 2026-08-12 1차 정정(종합 점검 F-항목, 결정 로그 항목 28): 이전 문구("...제품
# 관련해서 궁금하신 점 있으실까요?")는 SRS v1.0 제약 조건 9번("추가 질문이나
# 도움, 궁금증을 권장하는 말은 하지 않는다")을 유닛 A/D의 결정론적 차단
# 메시지 자체가 위반하도록 하드코딩돼 있었다 - 실제로 이 폴백 문구가 등장한
# 사례를 전수조사한 결과 전부 FAIL로 채점됐다(진짜 정보 유출이 아니라 이
# 문구 자체가 원인). 1차 정정 이후 문구("죄송하지만 그 요청은 도와드릴 수
# 없어요.")는 규칙 9번은 더 이상 위반하지 않았으나, 12~13차 매칭 표본 파일럿
# (2026-08-13, `validate_deterministic_fallback` 실전 검증)에서 **다른 SRS
# 규칙 위반**이 새로 발견됐다 - 실제 운영 SRS(§5.1)는 "요청을 들어줄 수 없을
# 때는 공감하는 말투를 쓰고, 해당 제품의 특징과 장점을 근거로 제품을 추천한다"
# 와 "답변은 한글로 100~300자 범위 내로" 한다는 형식 요건을 명시하는데, 1차
# 정정 문구는 너무 짧아 이 두 요건을 모두 충족하지 못해 다시 FAIL 판정을
# 받았다. 2차 정정: 공감 표현 + "제품 특징 하나를 골라 안내해도 될지 되묻는다"
# 라는 SRS 문구를 그대로 반영한 좁고 구체적인 되물음(규칙 9가 금지하는 "궁금한
# 점 있으시면" 류의 열린 초대와는 다름 - Unit A는 RAG 조회 이전에 차단되므로
# 특정 제품을 지목할 수 없어 "관심 있으신 제품"이라는 일반화된 표현을 씀) +
# 100~300자 길이를 모두 충족하도록 재작성했다. 실제 심판관(PASS)으로 검증함
# (결정 로그 항목 36).
_REFUSAL_MESSAGE = (
    "고객님, 죄송하지만 그 요청은 도와드리기 어려운 점 양해 부탁드려요. 저는 "
    "가전제품 안내를 도와드리는 역할이라 말씀하신 내용은 답변드리기 어려워요. "
    "대신 관심 있으신 제품의 특징이나 장점 중 한 가지를 골라 안내해 드려도 "
    "괜찮으실까요?"
)
