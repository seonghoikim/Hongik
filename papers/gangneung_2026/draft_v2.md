<!-- 강릉 KSII 추계학술발표대회 2026 제출용 2페이지 요약 논문 초안 (v2)
     ...
     - v1과의 차이: 연구자 피드백 "절대 한 그림으로 넣을 수가 없겠다. 전체 플로우를 이해할 수
       ...
       · 그림2_자가치유루프.png (1452×4281px, 세로) — §3.2에 배치. ①SRS 정의~⑤SRS 병합
         5단계 루프. 유닛 A~D 파이프라인은 "그림1 참조"로 축약해 중복을 없앰.
       · 그림3_수렴후검증.png (2352×465px, 가로) — 신설 §3.4에 배치. 헬드아웃→적응형
         재공격→교차 모델 검증→최종 보고.
       그림2가 유닛 A~D를 한 블록으로 축약한 덕에 세로 길이가 7425px → 4281px로 줄었다.
       그림1·3은 원래도 가로형이라 짧다.
...
- 그림 3개, 전부 이 폴더에 보존:
  ...
  - `그림2_자가치유루프.png`(1452×4281px, 세로) — §3.2. v1의 세로형 상세(1830×7425px)에서 유닛 A~D 블록을 "그림1 참조"로 축약해 절반 가까이 줄인 버전.
